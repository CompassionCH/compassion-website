...
class ProjectController(HomepageController):
    ...
    @route(
        ["/participant/<model('crowdfunding.participant'):participant>/"],
        type="http",
        auth="public",
        website=True,
    ...
    def _prepare_project_values(self, project, page, **kwargs):
        sponsorships, donations = self.get_sponsorships_and_donations(
            project.sponsorship_ids + project.csp_sponsorship_ids,
            project.invoice_line_ids,
        )
        ...
        return {
            ...
            "sponsorship_card_content": project.sponsorship_card_content(),
        ...
        sponsorships = [
            {
                "type": "sponsorship",
                "color": "blue",
                "text": _("%s %s sponsored")
                % (
                    sponsorship.child_id.preferred_name or _("A mum and her baby"),
                    _("was") if sponsorship.child_id else _("were"),
                ),
                "image": sponsorship.child_id.portrait,
                "benefactor": sponsorship.correspondent_id.preferred_name,
    ...
    def get_time_ago(self, given_date):
        return format_timedelta(
            given_date - datetime.today(), add_direction=True, locale=request.env.lang
        )
