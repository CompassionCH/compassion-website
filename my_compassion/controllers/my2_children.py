...
class MyCompassionChildrenController(http.Controller):
    ...
    @http.route('/my2/children/<model("compassion.child"):child>/details', type="http", auth="user", website=True)
    def my2_render_child_details_page(self, child, **kwargs):
        ...
        if child in children_sponsored_by_partner:
                ...
                breadcrumbs = [
                    {'name': 'Children', 'url': '/my2/children/', 'active': False},
                    {'name': child.preferred_name, 'url': '/my2/children/' + str(child.id), 'active': False},
                    {'name': 'Details', 'url': '/my2/children/' + str(child.id) + '/details', 'active': True},
                ]
