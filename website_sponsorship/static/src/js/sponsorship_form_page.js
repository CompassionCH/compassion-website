odoo.define("website_sponsorship.sponsorship_form", function (require) {
    "use strict";

    var publicWidget = require("web.public.widget");

    publicWidget.registry.SponsorshipFormConfirmation = publicWidget.Widget.extend({
        ...
        setup_default_values: function () {
            ...
            const select_fields = ["partner_title", "partner_lang", "payment_mode_id"];
            select_fields.forEach((s_field) => {
                if (form_values[s_field]) $(`select[name='${s_field}']`).val(form_values[s_field]);
            });
            ...
            check_fields.forEach((c_field) => {
                if (form_values[c_field]) {
                    form_values[c_field].forEach((check_id) => {
                        $(`input[name='${c_field}'][value='${check_id}']`).attr("checked", "checked");
                    });
                }
            });
            const birthdate = form_values.partner_birthdate_date;
            if (birthdate) {
                $("input[name='partner_birthdate_date']").val(new Date(birthdate).toLocaleDateString());
            }
        },
