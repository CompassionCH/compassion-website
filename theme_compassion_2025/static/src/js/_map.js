...
odoo.define("theme_compassion_2025.map", function (require) {
    ...
    /**
     * A singleton promise to ensure the Google Maps API is loaded only once per page.
     * @type {Promise}
     */
    ...
    publicWidget.registry.GoogleMap = publicWidget.Widget.extend({
        ...
        async start() {
            ...
            const lat = this.$el.data("lat");
            const lng = this.$el.data("lng");
            const apiKey = this.$el.data("apiKey");
            const mapId = this.$el.data("customMapId") ? this.$el.data("customMapId") : "YOUR_MAP_ID"; // Use the custom map ID from the data attribute, falling back to a default (of google) if not provided.

            if (!apiKey) {
                console.error("Google Maps API key not found.");
                this.$el.addClass("map--load-failed");
                return;
            }

            try {
                await this._loadGoogleMaps(apiKey);
                this._initializeMap(lat, lng, mapId);
            } catch (error) {
        ...
        /**
         * Loads the Google Maps API using the most robust bootstrap loader method.
         * This creates a unique callback to avoid conflicts and is required for importLibrary.
         *
         * @private
         * @param {String} apiKey.
        ...
        _loadGoogleMaps(apiKey) {
            if (!googleMapsLoadPromise) {
                googleMapsLoadPromise = new Promise((resolve, reject) => {
                    ...
                    const scriptUrl = `https://maps.googleapis.com/maps/api/js?key=${apiKey}&libraries=marker&callback=Function.prototype`;
                    $.getScript(scriptUrl)
                        .done(() => resolve())
        ...
        /**
         * Initializes the map instance, creates the marker, and customizes controls.
         *
         * @private
         * @param {Number} lat
         * @param {Number} lng
         ...
         * This is necessary for elements dynamically added by the Google Maps API, which we want to customize.
         *
         * @private
        ...
        /**
         * Orchestrates all UI customizations for the map controls.
         * This is called once the map is idle.
         *
         * @private
                ...
                this._pollForElement(SELECTORS.STREET_VIEW_BUTTON, (streetViewButton) => {
                    ...
                    streetViewButton.addEventListener("mousedown", () => {
                        this._pollForElement(
                            SELECTORS.GRAPPED_PEGMAN,
                            (grappedPegman) => {
                                grappedPegman.className = "grapped-pogman-container";
                                grappedPegman.querySelector("img").className = "grapped-pogman";

                                this._updateImagesInContainer(grappedPegman, svgDataUrl);
                            },
                            document.body
                        );
                    });
        ...
        /**
         * Builds a URL-encoded data URL from a the custom pagman SVG.
         *
         * @private
